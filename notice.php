...
$robot = Robot::init('192.168.249.104',8090);
...
class Robot{
    ...
    public function remote(){
        ...
        $param = [//若想使用同步处理，也就是你接收完事件要如何处理，那么你就要完善下面这个数组
            ...
            "robot_wxid" => isset($_REQUEST['robot_wxid']) ? trim($_REQUEST['robot_wxid']) : 'wxid_rqn31uwmjug922',//wxid_6mkmsto8tyvf52 wxid_5hxa04j4z6pg22
    ...
    public function response($request){
        ...
        file_put_contents("resp.txt",'events----'.$request['event'].PHP_EOL, FILE_APPEND);
    ...
    public function replay_text($request){
        ...
        if($this->startWith($msg,'复读')){
            file_put_contents("resp.txt",'replay_text----'.'走了'.PHP_EOL, FILE_APPEND);
